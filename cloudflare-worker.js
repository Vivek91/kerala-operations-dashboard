...
async function saveFile(env,name,request){
  ...
  const filename=String(file.name||"upload.xlsx"),contentType=String(file.type||"application/octet-stream");
...
async function mapping(env,method,request){
  ...
  const body=await request.json(),input=body&&body.mapping&&typeof body.mapping==="object"?body.mapping:{},out={};
